...
| Part | Sources | Notes |
| --- | --- | --- |
| Engine | `engine/common`, `imagelib`, `soundlib`, `server`, `http`, `public`, `platform/posix` (minus `sys_posix.c`, crash handlers and `lib_posix.c`), `xash/platform_ps5/lib_ps5.c`, `library_suffix`, bundled bzip2 | `XASH_DEDICATED=1 XASH_STATIC_LIBS=1`, `TIMER_POSIX`, `XASH_MESSAGEBOX=99` (own message box); the Phase 6 hybrid backend preserves these static entries |
...
`COM_LoadLibrary("server")` resolve through the static table in `lib_ps5.c`.
Unknown module names use the Phase 6 application-owned PRX path; see
`PRX_LOADER_PHASE6.md`. The normal Phase 5 regression still packages no probe
PRX and therefore proves the hybrid backend did not disturb this gate.
