...
typedef struct GoldSrcBrushPlan {
    uint32_t entity_indices[GOLDSRC_BRUSH_INSTANCE_COUNT];
    uint32_t draw_counts[GOLDSRC_BRUSH_INSTANCE_COUNT];
    uint32_t index_counts[GOLDSRC_BRUSH_INSTANCE_COUNT];
    uint32_t source_render_modes[GOLDSRC_BRUSH_INSTANCE_COUNT];
    uint32_t classname_hashes[GOLDSRC_BRUSH_INSTANCE_COUNT];
} GoldSrcBrushPlan;

typedef struct GoldSrcBrushFrame {
    const uint32_t *constant_tables[GOLDSRC_BRUSH_INSTANCE_COUNT];
